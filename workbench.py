...
REQUIRED_SERVICE_FUNCTIONS = (
    ...
    "list_daily_reviews", "daily_review_summary", "daily_review_detail", "start_daily_review",
)
...
def daily_review() -> None:
    ...
    st.caption("所有数据来自本地版本化复盘快照；页面不抓取实时行情，缺数据标注「未覆盖」，离线演示快照明确标记 synthetic_demo。")

    reviews = service.list_daily_reviews()
    if not reviews:
        st.info("尚未发现本地市场复盘快照。可运行 `scripts\\collect_daily_review.py` 生成离线演示快照，"
                "或通过 POST `/api/daily-review/acquire` 触发。")
        return

    labels = {f"{item['review_as_of']} · {item['id']}": item["id"] for item in reviews}
    selected = st.selectbox("复盘快照", list(labels), key="daily_review_picker")
