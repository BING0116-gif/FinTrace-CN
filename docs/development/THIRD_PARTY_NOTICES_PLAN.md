...
> v3.1 变更：MinerU 许可证从"AGPL-3.0"改为"待核实"（需以提交时官方源复核为准）。
...
| 项目 | 计划用途 | 合规要点 |
|---|---|---|
| MinerU | 复杂版式 PDF 结构化（可选 parser 后端） | **许可证待核实**（需以提交时官方源复核为准）；必须 extras 可选安装（`pip install fintrace[mineru]` 式隔离），不进入默认路径，且不得链接其闭源在线服务 |
