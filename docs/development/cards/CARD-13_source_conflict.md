...
| 属性 | 值 |
|---|---|
| 优先级 | P1（时间不足时降级为"记录冲突+阻断"最小实现） |
| 竞赛阶段 | 初赛版（10/12–10/15） |
| 对应赛题 | 赛题2/5（重述与更正场景）；数据准确性 |
| 依赖 | CARD-01（多文档）、CARD-04（重述信号） |
| 实现复杂度 | 低中（约 2 人日） |
| 状态 | ☐ 未开始 |
...
新建 `src/cn/conflict.py`：当同一指标同一期间出现多个来源值（原披露 vs 更正公告 vs 重述 vs 第三方数据源）时，按明确优先级规则裁决，全程记录，永不静默覆盖；无法确定时交 Validator 阻断或降级。
...
四级优先级正反例；链式更正；同级矛盾阻断；舍入容差不误报；scope 不同不误判冲突。
